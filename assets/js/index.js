...
swal( {  title: "Kullanıcı adı girmelisiniz.",   text: "Gerekli alanları doldurunuz.",  
 type: "warning",   showCancelButton: false, timer:2000, showConfirmButton: false});
setTimeout(function(){
document.login_form.username.focus(); }, 2100);
    
    return false; 
    } 
    if (document.login_form.password.value == "" ){ 
swal( {  title: "Şifre girmelisiniz.",   text: "Gerekli alanları doldurunuz.",  
 type: "warning",   showCancelButton: false, timer:2000, showConfirmButton: false}); 
setTimeout(function(){
document.login_form.password.focus(); }, 2100);    return false; 
    } 
...
$('body').on('keydown', 'input, select, textarea', function(e) {
    ...
    function validate_login_form(form, data) {
      if (data.user_username == "") {
        ...
        addFormError(form["user_username"], 'Lütfen kullanıcı adınızı girin.');
        return false; // stop the script if validation is triggered
      }

      if (data.user_password == "") {
        // if password variable is empty
        addFormError(form["user_password"], 'Lütfen şifrenizi girin.');
        return false; // stop the script if validation is triggered
      }
